<?php 
	...
	if (!isset($_GET['blog_id']) or empty($_GET['blog_id'])) {
	  ...
	  $blg_id=clean_values($con,$_GET['blog_id']);
	  page_visited($con,$blg_id);
	}
...
// ------------Selecting blog contents----------
	$query_select_blog=@"SELECT * FROM blogs WHERE (id=$blg_id AND status=1) LIMIT 1";
...
<div class="container-fluid">
	<nav class="navbar navbar-dark px-5">
		<p class="border-primary text-center pt-5" style="padding-top: 10px;">
			...
			<a href="blog.php" style="text-decoration: none;color: white; padding-left: 30px;">Back</a>
		</p>
		<h1 class="text-white pt-5" style="font-family: serif; font-style: italic;;">Lorem Ipsum Blogs</h1>
	</nav>
	...
	<div class="row px-5">
		<div style="background-color: #ffffff;border-radius: 30px;">
			...
			<div class="container-lg">
				<div class="row gy-3 row-cols-1 row-cols-sm-2 row-cols-md-4 row-col-lg-4">
					...
					<div class="col blogImg">
		              <img src="assests/images/<?php echo($img_rows['image_name']); ?>" class="gallery-item" alt="<?php echo($img_rows['img_alt_text']); ?>">
		           </div>
		       <?php } ?>
				</div>
			</div>
			<div>
				<p class="blockquote gy-4" style="font-style: italic;"><?php echo $rows['short_desc']; ?></p>
			</div>
			<div >
			...
			<div class="viewBlogIcons py-2 tableViews" align="center">
				<i class="hideContent" id="userId"><?php echo($ip); ?></i>
				<i class="hideContent" id="blogId"><?php echo($blg_id); ?></i>
		  		<i class="fa fa-eye"> &nbsp;<?php echo $rows['blog_view']; ?></i>
		  		<i class="fa fa-thumbs-up" id="blogLike"> &nbsp;<?php echo $rows['blog_like']; ?></i>
		  		<i class="fa fa-thumbs-down" id="blogDisLike"> &nbsp;<?php echo $rows['blog_dislike']; ?></i>
				...
				<div class="col-sm-8 col-md-8 col-lg-8 col-xs-12 py-3">
					<div class="card" id="commentSection">
					  ...
					  <div class="card-body">
						...
						<div class="mb-3">
							<input type="hidden" name="comment_blog_id" value="<?php echo($blg_id); ?>">
						  <label for="text" class="form-label">Full Name</label>
						 <input class="form-control form-control-lg" type="text" placeholder="lorem ipsum" name="commenter_name">
						</div>
						<div class="mb-3">
						  <label for="email" class="form-label">Email address</label>
						 <input class="form-control form-control-lg" type="text" placeholder="lorem.ipsum@gmail.com" name="commenter_email">
						</div>
						<div class="mb-3">
			...
			<?php 
				$query_select_blog_comment=@"SELECT * FROM blog_comments WHERE (blog_id=$blg_id AND status=1) ORDER BY id DESC";
			  		  ...
			  		  <div class="collapse tableViews" id="commentReplies<?php echo($comment_id); ?>">
							...
							<div class="card col-sm-8 col-md-8 col-lg-8 col-xs-12">
					  			<form class="" method="post" action="">
					  				<input type="hidden" name="reply_blog_id" value="<?php echo($blg_id); ?>">
					  				<input type="hidden" name="reply_comment_id" value="<?php echo($select_blog_comment_rows['id']); ?>">
					  				<div class="mb-3">
					  					<label for="userName" class="form-label">Name:</label>
					  					<input type="text" name="replier_name" placeholder="lorem Ipsum" class="form-control" id="userName">
					  				</div>
					  				<div class="mb-3">
					  					<label for="userEmail" class="form-label">Email:</label>
					  					<input type="email" name="replier_email" placeholder="lorem.Ipsum@gmail.com" class="form-control" >
					  				</div>
					  				<div class="mb-3">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('#blogLike').click(function(){
			...
			$.ajax({
			url: "assests/includes/functions.inc.php",
			type: 'post',
			data: {	
				userId: userId,
				blogId: blogId,
